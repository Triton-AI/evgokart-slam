    ...
    def camera_model(self):
        ...
        with dai.Device() as device:
            ...
            while True:
                ...
                depthFrame = depthData.getFrame() # depthFrame values are in millimeters
                ...
                for xyxy in results.xyxy[0]:
                    ...
                    text.rectangle(depthFrame, (int(x1), int(y1)), (int(x2), int(y2)))
                    
                    text.putText(depthFrame, "X: " + ("{:.1f}m".format(spa_x/1000) if not math.isnan(spa_x) else "--"), (x + 10, y + 20))
                    text.putText(depthFrame, "Y: " + ("{:.1f}m".format(spa_y/1000) if not math.isnan(spa_y) else "--"), (x + 10, y + 35))
                    text.putText(depthFrame, "Z: " + ("{:.1f}m".format(spa_z/1000) if not math.isnan(spa_z) else "--"), (x + 10, y + 50))
